...
Writes are not free. The first direct file write, shell command, or git operation asks the user to enable writes for the conversation. The grant enables the selected provider's native writable mode and lasts until the user revokes it or restarts KPM, so it is a decision about trusting the conversation, not a confirmation of one edit. In agent execution, writes remain scoped to an isolated worktree; the developer's working branches are untouched until they merge.
...
**What this obliges us to keep:** the unlock must be visible while it is active and revocable at any time — the chat header shows when its conversation is unlocked and revokes on click. KPM-controlled file tools keep credential and secret paths denied.

**Providers differ, and we say so rather than pretending otherwise.** Claude and pi can pause a running turn to ask. Codex cannot: its SDK has no approval channel, so its first blocked write fails, the user is asked afterwards, and the thread resumes in native `workspace-write` mode on the next turn. This is recorded as `inTurnWriteApproval` in `src/shared/providerCapabilities.ts` so the difference is declared, not discovered. Claude, Codex, and pi keep ownership of their native tool and sandbox behavior; KPM does not wrap one provider in another to manufacture identical semantics. If in-turn Codex approvals ever justify it, the path is `codex app-server`, which exposes `execCommandApproval` / `applyPatchApproval` over JSON-RPC — at the cost of owning a protocol client.

The same asymmetry applies to reads. Claude denies reads of credential and secret roots through its SDK options, and pi denies them in its own tool call gate; Codex's native shell and file tools accept no read deny-list at all, because its sandbox governs write scope, not reads. KPM's own file access resolves through `checkRealpathAccess` (`src/main/services/files/pathSecurity.ts`), which refuses a path whose realpath lands inside a protected root (`getDeniedRealpathRootsAsync`); `FileExplorerService` is the confirmed caller, reached by the `list_project_files` and `delete_project_file` MCP tools available to every provider including Codex. Other path-touching KPM tools (document read/edit/update, `git_read`, the spill-file reader) validate by lexical containment instead — a relative path with no `..`, or resolution against a known root — not the realpath deny-list, so this specific guard does not cover them.
