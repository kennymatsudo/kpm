...
- **Rebuild a parent table only with `foreignKeysOff: true`.** Every `up` runs in a transaction, and SQLite ignores `PRAGMA foreign_keys = OFF` inside one, so `DROP TABLE` on a table other tables reference with `ON DELETE CASCADE` deletes those child rows, and nothing reports it. Setting `foreignKeysOff: true` on the migration makes `applyMigration` switch foreign keys off before the transaction, run `PRAGMA foreign_key_check` before commit, and turn them back on. Older migrations that write the pragma inline do not work; do not copy them. Dropping a leaf table is safe. `126_widen_dev_session_paused_reason` is the template for a rebuild; migrations 056 and 125 show how to avoid one.
...
For a migration test, migrate to just before the new one, seed rows as they existed then, run the new migration's `up`, and assert the data survived and `PRAGMA foreign_key_check` returns nothing. Apply it through `applyMigration` so the runner's foreign-key handling is exercised. The `126_widen_dev_session_paused_reason` block in `migrations.test.ts` is the template.
