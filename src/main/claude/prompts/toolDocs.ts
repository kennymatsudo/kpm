...
- **Git state:** use \`git_read\` for git history, diffs, status, blame, branches, and \`merge-base\`. Raw \`git\` in Bash is blocked in chat — \`git_read\` is the only git path, and it is read-only.
...
- **KPM changes:** propose changes with the appropriate change tool. Use \`modify_plan\` for plan mutations, \`propose_context_edit\` for the project context file, and document proposal tools for project files.
