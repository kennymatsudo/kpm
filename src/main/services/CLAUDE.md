...
- `UpdateEventBus` — Cross-service update broadcast helper. Producers today: `ReviewPollService` (`pr_changed`), `ScheduledLoopRunnerService` (`loop_finding`), `RepoWatcherService` (`branch_changed`, suppressed), and `automationPhaseMachine` (`board_agent`). `ticket_changed` and `generic_update` have rules but no producers.
